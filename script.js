...
function init(queue) {
  document
    ...
    .addEventListener("click", addClient.bind(null, queue));
  document
    .getElementById("attend-client")
    .addEventListener("click", nextClient.bind(null, queue));
  printClients(queue);
}

init(
  new Queue(
    new Client("Tuco Salamanca"),
    new Client("Jesse Pinkman"),
    new Client("Hank Schreder"),
    new Client("Skyler White"),
    new Client("Heisenberg")
  )
);
